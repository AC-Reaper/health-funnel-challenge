...
| review-016-brief-compliance-pay | Brief-compliance pay | Open (fix applied) | Closeout re-review at `b3a1d24`: I002 and N001 resolved; typecheck, 255 tests, build, `db:validate`, diff-check, raw-query grep, and preview seed/by-session/manual `/pay` smoke pass. I001's last stragglers in `docs/02-architecture.md` (v1→v2 status blurb, decision gate, §0 header, R8 row, "pre-seeded paid sessionId dropped") cleared to ADR-001…019 / two-path truth in the closeout follow-up; awaiting Codex final confirmation before merge. |
