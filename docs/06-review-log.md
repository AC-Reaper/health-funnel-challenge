...
| review-010-delivery-compliance | Delivery compliance | Open | `feature/delivery-compliance-hardening` ships the /pay submitted gate (`409 NOT_SUBMITTED` at API + state machine), README submission block + paid-test cURL + email template, `docs/03 §2.1` Logical model mapping, `docs/04` error rows, `/results` full trust footer, and `docs/07` final flip. 206 → 210 tests green; typecheck + build clean. Awaits Codex (optional). |
