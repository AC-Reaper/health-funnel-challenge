...
| review-009-security-hardening | Security hardening | Open | `feature/security-hardening` ships same-origin guard (`lib/api/same-origin.ts`), Idempotency-Key printable-ASCII (`lib/api/idempotency-key.ts`), `docs/08-security-hardening.md`, and 18 new vitest cases. 184 → 202 tests green; typecheck + build + db:validate clean. Awaits Codex. |
