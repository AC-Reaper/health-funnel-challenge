...
| review-009-security-hardening | Security hardening | Open | Re-reviewed `feature/security-hardening` at `adafa91`: typecheck, 206 tests, build, db:validate, and diff-check pass. N001 same-origin precision is resolved; I001 remains open because `docs/08` / resolved log still claim a nonexistent `lib/db.ts` `$queryRaw` warm-up callsite. |
