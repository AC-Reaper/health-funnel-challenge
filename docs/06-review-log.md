...
| review-014-rate-limit | Rate limiting | Open | Reviewed `feature/rate-limit` at `b2403a1`: no Blocking or Important findings. `typecheck`, 240 tests, build, `db:validate`, diff-check, raw-query grep, and preview 429 smoke pass. One Nice-to-have remains: tighten the "salted" hash comment and stale `lib/payment.ts` line citation in `docs/08`. |
