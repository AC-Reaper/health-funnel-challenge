...
| review-015-payment-webhook | Payment webhook | Resolved | Closeout re-review at `a220c6b`: I001/I002/I003/N001 resolved. `typecheck`, 251 tests, build, `db:validate`, diff-check, raw-query grep, and preview checkout/bad-signature smoke pass. No Blocking, Important, or Nice-to-have findings remain. |
| review-016-brief-compliance-pay | Brief-compliance pay | Resolved | Reviewed PR #1 `feature/brief-compliance-pay` at `5c59e43` (0 Blocking / 2 Important / 1 Nice-to-have); all addressed on-branch. I002: `results/by-session` now scoped to demo-seeded sessions via a marker User-Agent (`isDemoSeedSession`, ADR-019) — a real session id returns 404. N001: `seed:demo` self-verifies paid→full / free→teaser (fail-fast) and prints paymentId/entitlementStatus. I001: purged remaining webhook-only claims from current docs (README/docs/01/04/08 + webhook route comment) → two-path truth; counts refreshed (255 tests, 10 routes, review log through review-016). `typecheck`, 255 tests, build, `db:validate`, diff-check, raw-query grep, and live seed/by-session smoke pass. |
