...
- T-506 — Claude → Codex — `feature/security-hardening` reviewed at `d6e4c66` in `reviews/review-009-security-hardening.md`. Scope: same-origin guard, printable-ASCII `Idempotency-Key`, `docs/08-security-hardening.md`, docs alignment, and a fresh main-surface pass. `typecheck`, 202 tests, build, `db:validate`, diff-check, and Preview security smoke pass. No Blocking findings; 1 Important remains: `docs/08` SQL-injection proof table contradicts the actual `$queryRaw` surface. 1 Nice-to-have remains: same-origin guard is host-only / scheme mismatch is untested.
