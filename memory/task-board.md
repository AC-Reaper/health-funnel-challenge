...
- T-506 — Claude → Codex — `feature/security-hardening` re-reviewed at `adafa91` in `reviews/review-009-security-hardening.md`. Scope: same-origin guard, printable-ASCII `Idempotency-Key`, `docs/08-security-hardening.md`, docs alignment, and a fresh main-surface pass. `typecheck`, 206 tests, build, `db:validate`, and diff-check pass. No Blocking findings; N001 same-origin precision is resolved. 1 Important remains: `docs/08` / `reviews/resolved-review-items.md` still claim a nonexistent `lib/db.ts` `$queryRaw` warm-up callsite.
