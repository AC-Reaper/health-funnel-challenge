...
Source: `reviews/review-016-brief-compliance-pay.md` (0 Blocking, 2 Important, 1 Nice-to-have). Direction accepted (restore the brief's mock `/pay` beside the signed webhook); all findings fixed on-branch under ADR-019.
...
- **I001 — current evaluator-facing docs still asserted the webhook-only model** → swept to the two-path truth: `app/api/v1/payments/webhook/route.ts` header comment ("ONLY path that grants" → "production grant path; mock `/pay` is a parallel secret-free grant"); `README.md` (review log `review-013`→`review-016`, "granted only by webhook" prose → two-path, `250`→`255` tests en+zh, "7 个 /api/v1 路由"→`10`, ADR-001…018→…019, `by-session` flagged seeded-demo-only); `docs/01-requirements.md` (R-014 two-path + out-of-scope sessionId bullet annotated demo-scoped); `docs/04-api-design.md` (header ADR-001…019 + "ten routes", same-origin list +`/pay`, §5b demo-seeded scope); `docs/08-security-hardening.md` (§3.6 demo-read paragraph now scoped to the marker UA). Historical task lists / review files left as-is.
