...
Source: `reviews/review-016-brief-compliance-pay.md` (0 Blocking, 2 Important, 1 Nice-to-have). Direction accepted (restore the brief's mock `/pay` beside the signed webhook). I002 and N001 are fixed on-branch under ADR-019; I001 remains partially open pending a final `docs/02-architecture.md` cleanup.
...
Codex closeout re-review at `b3a1d24`: I002 and N001 are verified resolved, including preview smoke on `https://project-u415a-nrsua6m2q-jackz1.vercel.app/`. I001 is only partially resolved: README/docs/01/docs/04/docs/08 and the webhook route comment are aligned, but `docs/02-architecture.md` still carries stale ADR-001…017 / pre-ADR-018 status text. Keep review-016 Open until docs/02 is cleaned up.
