...
## review-015 (payment-webhook) findings

Source: `reviews/review-015-payment-webhook.md` (0 Blocking, 3 Important, 1 Nice-to-have).

- **I001 — checkout bypassed the Zod/body contract** → `app/api/v1/payments/checkout/route.ts` now runs `parseJsonBody(req, z.object({}).strict(), requestId)` after the rate-limit check, mirroring `/sessions` and `/submit`. Malformed JSON / wrong Content-Type → 400, unknown keys → 422 via the shared helper, whose behaviour is regression-tested in `tests/lib/api/parse-body.test.ts` (the repo has no route-test harness; same convention as the other two empty-body routes).
- **I002 — webhook idempotencyKey bypassed the printable-ASCII hardening** → `WEBHOOK_PAYLOAD_SCHEMA.idempotencyKey` now reuses `IDEMPOTENCY_KEY_SCHEMA` (review-009) instead of a bare 1-128 string. New `tests/lib/payment-webhook.test.ts` cases assert newline / NUL / non-ASCII keys reject and a clean key passes.
- **I003 — current docs/memory still advertised the removed `POST /api/v1/pay`** → purged from current claims: `docs/04` (browser-routes note, same-origin list, 429 row, and the bottom cURL block — which 404'd — now checkout + openssl-signed webhook), `docs/02` (§0 ADR-006 row superseded-by-ADR-017, §1 MVP scope, §2 edge paths, §5 transaction intro, §9 rate-limit row, R3 risk), `docs/08` (§1 attack surface + `/checkout`, §2 Zod proof now cites the live checkout route + webhook schema instead of the deleted `app/api/v1/pay/route.ts`, §2 control rows + `$queryRaw` sessionId provenance, §3 test-proof rows, §3.5 route list + limits), `docs/03` (payment table + index wording), `memory/shared-memory.md` (Final Goal, Delivery Decisions, MVP flow 8-9, artefacts), `memory/decisions.md` (ADR-006 superseded pointer + rate-limit note). Historical task lists / review files left as-is.
- **N001 — signature-valid event for a bad session surfaced as 500** → the webhook now pre-reads the session after payload validation: unknown → 404 NOT_FOUND, not-submitted → 409 NOT_SUBMITTED; already-paid still flows through the unchanged `processPayment` no-op. Genuine post-check races still fall to 500 (correct).

Verification after fixes: `tsc --noEmit` clean, `npm test` 250 → 251 green, `next build` clean, `npx prisma validate` clean, `git diff --check` clean, and `grep -rn "api/v1/pay" docs README.md memory` returns only historical task lists / review files / the `feature/pay-subscription` branch name.

Codex re-review at `a220c6b`: I001/I002/I003/N001 verified resolved. Additional checks: raw-query grep still returns exactly one application callsite (`lib/payment.ts:200`); preview smoke on `https://project-u415a-24xgpmpdq-jackz1.vercel.app/` confirms old `/api/v1/pay` 404, bad-signature webhook 401, checkout extra-field body 422, checkout `{}` pending, and checkout leaves submitted results in `teaser`. Direct signed-success webhook cURL was not verified against preview because the local `.env` secret differs from the Vercel secret; signed-success is covered by code/tests/server-action design.

## review-016 (brief-compliance-pay) findings

Source: `reviews/review-016-brief-compliance-pay.md` (0 Blocking, 2 Important, 1 Nice-to-have). Direction accepted (restore the brief's mock `/pay` beside the signed webhook). I001/I002/N001 are fixed on-branch under ADR-019; review-016 is Resolved after final confirmation of the `docs/02-architecture.md` cleanup.

- **I002 — `GET /results/by-session` exposed all submitted sessions by bare UUID** → scoped to demo-seeded sessions only. `lib/session.ts` now exports `DEMO_SEED_USER_AGENT` + a pure `isDemoSeedSession(ua)`; `app/api/v1/results/by-session/route.ts` returns `404 NOT_FOUND` when the session is null **or** not a demo-seed session (collapsed into one branch so a real session's existence is never confirmed); `scripts/seed-demo.sh` sends `User-Agent: health-funnel-demo-seed/1.0` on the `POST /api/v1/sessions` create call (the only call whose UA is persisted). The predicate is unit-tested in `tests/lib/session.test.ts` (marker → true; null / normal browser UA / near-miss → false). Net: a leaked/guessed UUID of a real visitor's session is no longer bearer read-access; the cookie stays the real credential.
- **N001 — `seed:demo` did not prove the promised contrast** → after minting both sessions the script calls `GET /results/by-session` for each and `exit 1`s unless paid is `kind:"full"` and free is `kind:"teaser"`, and now prints the paid `paymentId` / `entitlementStatus` (making the README/email copy honest).
- **I001 — current evaluator-facing docs still asserted the webhook-only model** → swept to the two-path truth: `app/api/v1/payments/webhook/route.ts` header comment ("ONLY path that grants" → "production grant path; mock `/pay` is a parallel secret-free grant"); `README.md` (review log `review-013`→`review-016`, "granted only by webhook" prose → two-path, `250`→`255` tests en+zh, "7 个 /api/v1 路由"→`10`, ADR-001…018→…019, `by-session` flagged seeded-demo-only); `docs/01-requirements.md` (R-014 two-path + out-of-scope sessionId bullet annotated demo-scoped); `docs/04-api-design.md` (header ADR-001…019 + "ten routes", same-origin list +`/pay`, §5b demo-seeded scope); `docs/08-security-hardening.md` (§3.6 demo-read paragraph now scoped to the marker UA). Closeout follow-up (flagged by Codex's `b3a1d24` re-review): `docs/02-architecture.md` stragglers cleared — the v1→v2 status blurb ("pre-seeded paid sessionId dropped" → now provided via seed + by-session, ADR-018/019), the **Decision gate** + §0 header (ADR-001…017 → …019), and the R8 risk row (ADR-001…017 → …019). Historical task lists / review files left as-is.

Verification after fixes: `tsc --noEmit` clean, `npm test` 251 → 255 green, `next build` clean (route manifest still has `/api/v1/pay` + `/api/v1/results/by-session`), `npx prisma validate` clean, `git diff --check` clean, raw-query grep still exactly `lib/payment.ts:200`. Live smoke vs Supabase: `npm run seed:demo` self-verify passes (paid=full, free=teaser) and prints paymentId/entitlementStatus; a non-seeded submitted session id → `404` (the I002 fix); the seeded paid id → `full`, free id → `teaser`; manual cookie-jar `POST /api/v1/pay` still grants without `PAYMENT_WEBHOOK_SECRET`.

Codex closeout re-review at `b3a1d24`: I002 and N001 are verified resolved, including preview smoke on `https://project-u415a-nrsua6m2q-jackz1.vercel.app/`. Final confirmation after the closeout follow-up verifies the remaining I001 `docs/02-architecture.md` cleanup as well: the architecture doc now uses ADR-001…019 and the current seeded demo paid-session truth. review-016 is Resolved.
